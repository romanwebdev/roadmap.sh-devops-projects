...
const SERVICE_PORT = process.env.SERVICE_PORT || 3002;
...
async function registerWithConsul() {
  ...
  const res = await fetch(`http://${CONSUL_HOST}:8500/v1/agent/service/register`, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(payload)
  });

  console.log(res.ok ? `Registered with Consul as ${ownIp}` : 'Failed to register');
}
